...
/** Channel-safe descriptor for uploaded content referenced by a tool result. */
export interface OutgoingContentItem {
  /** Stable content reference generated by the gateway content store. */
  contentRef: string;
  /** Optional filename hint for preview/download rendering. */
  fileName?: string;
  /** Optional MIME type hint (e.g. image/png, application/pdf). */
  mimeType?: string;
  /** Optional byte length for UX display. */
  byteLength?: number;
  /** API path users/channels can use to download the content. */
  downloadPath?: string;
